...
import type { AdSpendMatrix } from "./useAdSpendMatrix";
...
type Props = {
  ...
  /** Today's total ad spend per product (SUM(spend) across campaigns) — computed server-side. */
  adSpendValues: Map<number, number>;
  /** Historical ad-spend matrix (SUM(spend) per day) — computed server-side. */
...
export function DashboardAdSpendDetailSection({
  products,
  adSpendValues,
  adSpendMatrix,
  onBack,
  ...
  const matrixByNmId = useMemo(
    () => new Map(adSpendMatrix.products.map((p) => [p.nmId, p.values])),
    [adSpendMatrix.products],
  );

  const pastDates = useMemo(() => {
    const result: string[] = [];
    for (const d of adSpendMatrix.dates) {
      if (d !== today) result.push(d);
    }
    return result;
  }, [adSpendMatrix.dates, today]);
  ...
  const dateTotals = useMemo(() => {
    ...
    return totals;
  }, [pastDates, products, adSpendValues, adSpendMatrix.products, adSpendMatrix.dates, today]);

  const matrixIdxByDate = useMemo(() => {
    const m = new Map<string, number>();
    adSpendMatrix.dates.forEach((d, i) => m.set(d, i));
    return m;
  }, [adSpendMatrix.dates]);
  ...
  const getPinnedCell = useCallback(
    (rowIdx: number): CellContent => {
      ...
      const v = adSpendValues.get(p.nmId);
      return moneyCell(v ?? null);
    },
    [products, adSpendValues],
  );
