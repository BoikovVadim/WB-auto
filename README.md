...
- scheduled WB Promotion sync now runs as a phased pipeline every 10 minutes by default: `inventory` -> `structure` -> `stats`, and each phase drains its full ordered pool through continuous global batching instead of campaign-per-run budgets; the cadence is env-driven through `WB_PROMOTION_SYNC_CRON`, so production can keep the expected 10-minute refresh without hardcoding a slower hourly trigger; by default the background full sync refreshes monthly frequency plus product-level `cmp.wildberries.ru` enrichment when the needed sources are available, while JAM/search-text backfill stays on its dedicated nightly/manual lane so ordinary promotion refreshes do not silently consume the shared `product/search-texts` quota, and the canonical stats lookback now keeps a 30-day window so the product date filter can project a full calendar month
- monthly frequency now has an explicit daily refresh lane (`WB_PROMOTION_MONTHLY_FREQUENCY_SYNC_CRON`, default `0 4 * * *` UTC / 07:00 MSK): this keeps the 30-day frequency snapshot fresh even when full promotion sync cadence is tuned independently; product advertising reads frequency strictly from WB analytics report queries via exact normalized query matches, so punctuation/case normalization still works but stem/name fallback no longer fabricates values for clusters that have no matching report rows
...
- product advertising interactive reads now follow one backend/Postgres truth rule across `advertising-sheet`, `workspace`, `workspace-cluster-table`, and `workspace-cluster-queries`: explicit `startDate/endDate` requests read exact/current data for that requested range from PostgreSQL-backed projections, and when exact data is absent the backend returns machine-readable empty/unavailable readiness for that same period instead of silently reviving `closest_range`, `most_recent`, sheet-derived shell state, or client-side reconstruction
- product advertising read/write responsibilities are now explicit at the contract level: filtering, sorting, pagination, totals, filter counts, campaign ordering semantics, readiness, and revision/invalidation are backend-owned; frontend detail keeps only UI state and refresh triggers, while the separate `/live` endpoint remains diagnostic-only instead of serving as a hidden fallback for the user click path
...
- dashboard frontend polish now treats bootstrap, section switches, and large tables as separate performance surfaces: shell refresh is staged instead of waiting for one all-or-nothing bundle, product-list readiness/warmup only runs while the `products` list is active, product advertising truth caches stay memory-only while durable browser persistence is limited to UX preferences, and the biggest product/method tables use windowed rendering so fallback DOM does not exceed the main path
...
7. Optional monthly frequency cadence override: set `WB_PROMOTION_MONTHLY_FREQUENCY_SYNC_CRON` (default `0 4 * * *`, daily 07:00 MSK) for a dedicated frequency refresh
8. JAM safety defaults: keep `WB_PROMOTION_ENABLE_JAM_IN_FULL_SYNC=false` and `WB_PROMOTION_JAM_BACKFILL_LOOP_ENABLED=false` so PM2 restarts and regular promotion syncs do not spend the shared WB `product/search-texts` quota; use the dedicated nightly JAM cron plus explicit manual backfill when you actually want to catch up historical days
9. Optional guarded write key: set `WB_CLUSTERS_WRITE_API_KEY` on the backend and the matching `VITE_WB_CLUSTERS_WRITE_API_KEY` on the frontend when you want mutating `wb-clusters` calls to require a shared key in addition to the dashboard write-intent header
10. Leave `VITE_API_BASE_URL` empty for the default relative `/api` dev+prod routing, or override it explicitly when needed
...
- explicit user `GET` with range/filter/page/sort must read exact/current backend/Postgres truth for that request, or return a machine-readable empty/unavailable state for the requested scope
- degraded/latest/historical views must be explicit machine-readable modes or separate endpoints, never silent `closest/latest/archive` fallback in the normal user read path
- frontend caches may hydrate UX, but they must not compute or persist business truth that can outlive a refresh
- frontend should render backend-owned DTOs and invalidate them after writes instead of carrying a parallel read-model engine in hooks/storage
