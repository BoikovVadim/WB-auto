...
Оба дампа снимаются единым скриптом `scripts/oqqi-db-transfer.sh` (в репозитории):
`… initial` — первичная поставка (как выше); `… cutover` — догоняющий рефреш
бизнес-данных в день переключения (5 крупных таблиц исключены целиком, чтобы не
затереть накопленное экземпляром Oqqi). Скрипт стримит дамп с прода и проверяет
сигнатуру; команду восстановления печатает в конце.
...
- **Префикс пути.** Дефолт прод-сборки — подпуть **`/wb/`** (legacy `legendgames.space/wb`).
  Для корня домена `sales.oqqi.io` выставьте при сборке **`VITE_BASE_PATH=/`** (env из
  корневого `.env`) и пересоберите — ассеты пойдут от корня. `outDir: Frontend/build`.
  API-базу фронт берёт из `VITE_API_BASE_URL` (если задана) или относительным путём.
