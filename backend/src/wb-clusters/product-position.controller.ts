 ...
 * Замер — ФОНОВЫЙ (браузер-рендер, холодный старт ~75с): фронт триггерит и поллит
 * GET positions. Глобальный обход оркестрирует фронт по текущему порядку таблицы.
 * Сиблинг WbClustersController/ProductCpoController.
 */
...
export class ProductPositionController {
  ...
  /** Последние замеры мест по всем кластерам товара (цель поллинга). */
  @Get(":nmId/positions")
  ...
  /** Запустить замер места по одному кластеру (фоном); результат забирается поллингом. */
  ...
  runCluster(
    ...
    return this.productPositionService.startClusterProbe(nmId, name);
  }
}
