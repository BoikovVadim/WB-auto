 ...
 * (кластеры с JAM/halo-заказами тоже участвуют). Пол 100 показов гасит шум малых кластеров.
 * Потолок выходит щедрым, но это безопасно: ставка ползёт по +10% и достигает топ-4 ЗАДОЛГО
 * до потолка — потолок остаётся лишь предохранителем окупаемости. Возвращает долю: 0.0123 = 1.23%.
 */
...
export interface BidEngineParams {
  ...
  maxWbBid: number;
  /**
   * РАЗГОН: пока не добрались до топ-4 — поднимаем на фикс-шаг = эта доля от МИНИМАЛЬНОЙ
   * ставки кластера за круг (0.10 = +10% от мин). Напр. мин 370 → шаг +37₽ каждый круг,
   * независимо от текущей ставки. Так разгон ровный и предсказуемый.
   */
  coarsePct: number;
  /**
