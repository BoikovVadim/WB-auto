 ...
 * не достигнет Макс СРО (порог терпения). CPO = accruedSpend / max(заказы РК, JAM) — max даёт
 * кластеру «зачёт» за halo-заказы. Макс СРО (maxCpo) — уже ×2 от целевого (константа метрики).
 *
 * Состояния: 'learning' (0 заказов, расход < Макс — копим) | 'active' (CPO ≤ Макс) |
 * 'excluded_high' (0 заказов и расход ≥ Макс — надёжно плохой; ИЛИ есть заказы и CPO > Макс).
...
export function decideForClusterV2(
  ...
  if (ordered === 0) {
    // Заказов нет: пока накопленный расход не дорос до Макс СРО (= 1× порога; Макс СРО сам уже
    // = обычный СРО × 2) — НЕ судим (шум малой выборки), держим в LEARNING. Потратили на кластер
    // как на один заказ по максимальной цене и заказа нет → надёжно плохой → исключаем.
    if (input.accruedSpend < maxCpo) {
      desiredActive = true;
