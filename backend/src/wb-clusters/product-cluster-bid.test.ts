...
describe("computeDesiredBid (P>4 → +10% от мин. ставки, P≤4 → −10₽)", () => {
  ...
  it("ставка ВСЕГДА целая при дробном bid_cap (WB отклоняет дробные)", () => {
    // bid_cap = МаксСРО×1000×CR — дробный; раньше at_cap/clamp возвращали его как есть → WB
    // «incorrect request body» и падал весь батч. Теперь потолок floor'ится, ставка целая.
    const up = computeDesiredBid({ position: 8, currentBid: 750, bidCap: 755.2847 }, PARAMS);
    expect(Number.isInteger(up.bid)).toBe(true);
    expect(up.bid).toBe(755); // 750+10=760 > floor(755.28)=755 → clamp к потолку 755

    const atCap = computeDesiredBid({ position: 8, currentBid: 800, bidCap: 686.84 }, PARAMS);
    expect(Number.isInteger(atCap.bid)).toBe(true);
    expect(atCap.reason).toBe("at_cap");
    expect(atCap.bid).toBe(686); // floor(686.84)
  });

  it("дробный текущий бид (после старого бага) округляется вниз", () => {
    const r = computeDesiredBid({ position: 20, currentBid: 686.8447, bidCap: 5000 }, PARAMS);
    expect(Number.isInteger(r.bid)).toBe(true);
    expect(r.bid).toBe(696); // floor(686.84)=686 + 10 (вниз, чтобы не задрать выше потолка)
  });
});
