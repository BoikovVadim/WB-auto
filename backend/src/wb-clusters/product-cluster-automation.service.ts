 ...
 * Нет расхода и заказов («нет данных») кластер НЕ выбраковывает: причина прошлого
 * решения сохраняется (исключён по CPO → остаётся «искл. по CPO», даже когда расход
 * вытек из 30-дневного окна), а новый/активный без данных остаётся активным — даём шанс
 * накопить данные (первое появление в РК). Старый исход «выбыл» (dropped) по «нет данных»
...
export class ProductClusterAutomationService {
  ...
  private decideForCluster(
    ...
    if (effectiveCpo !== null) {
      ...
      // Нет свежих данных за 30 дней (ни расхода, ни заказов). НЕ выбраковываем кластер
      // по «нет данных» — сохраняем ПРИЧИНУ прошлого решения, а новым/активным даём шанс
      // набрать данные. Почему это важно: после исключения по CPO кластер перестаёт
      // тратить, и за ~30 дней его расход вытекает из окна → effectiveCpo становится null.
      // Раньше такой кластер сваливался в "dropped" («выбыло»), хотя на деле он
      // «искл. по CPO». Теперь причина сохраняется.
      if (manualProtected) {
        desiredActive = true;
        state = "manual_protected";
      } else if (prev?.state === "excluded_high" || isExcludedNow) {
        // Был исключён по CPO или сейчас исключён → причина «высокий CPO», не «нет данных».
        // WB-состояние не меняем больше, чем раньше (desiredActive=false, как у dropped).
        desiredActive = false;
        state = "excluded_high";
      } else {
        // Активен и без данных (первое появление в РК / временно без трафика) → оставляем
        // активным, даём шанс накопить данные. Дальше судьбу решает CPO. Это снимает
        // авто-исключение «пустых» кластеров, которое было у старого "dropped".
        desiredActive = true;
        state = "active";
      }
    }
