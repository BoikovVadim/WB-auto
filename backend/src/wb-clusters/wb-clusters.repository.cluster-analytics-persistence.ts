...
export abstract class WbClustersRepositoryClusterAnalyticsPersistence extends WbClustersRepositoryClusterStatsWrite {
  /**
   * Upserts daily cluster stats rows without ever deleting existing data.
   * Historical rows accumulate indefinitely in wb_cluster_daily_stats.
   * Each sync just updates the rows that WB returned — rows for dates WB
   * did not include are left untouched (no delete, no wipe).
   */
  async upsertClusterDailyStats(input: {
    advertId: number;
    nmId: number;
    rows: Array<{
      date: string;
