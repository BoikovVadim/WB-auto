...
export const appEnv = {
  ...
  // Освежение заказов за СЕГОДНЯ через Sales Funnel (Воронку). orderCount/orderSum
  // совпадают с кабинетом WB «Заказали товаров» (Statistics API занижал ~12% —
  // выкидывал неоплаченные заказы). ЧАСОВОЙ cron (минута :07): данные Воронки и так
  // обновляются раз в час, а полный каталог (~450 nmId, батчи по 20 с троттлом 25с,
  // лимит 3 req/min) тянется ~10 мин — чаще бессмысленно и упрёмся в 429. Запуски
  // не пересекаются (интервал >> прогона + гард в шедулере). Финализация за вчера и
  // сверка истории — на отдельных CSV-кронах, это не про сегодня.
  wbOrdersSyncCron: getOptionalEnv("WB_ORDERS_SYNC_CRON", "0 7 * * * *").trim() || "0 7 * * * *",
