...
## Cross-Context Truth Canon

- Explicit user `GET` with range/filter/page/sort must read exact/current backend/Postgres truth for that request, or return a machine-readable empty/unavailable state for the requested scope. Silent `closest/latest/archive` fallback is not allowed on the normal user read path.
- Degraded views are allowed only as explicit machine-readable modes such as `latest`, `historical_preview`, or `unavailable`, or through separate diagnostic/support endpoints.
- Frontend persistence is UX-only unless a cache entry is opaque, revision-bound, and always backend-revalidated. Browser state must not own business truth across refreshes.
- Frontend should render backend-owned DTOs. Totals, filter counts, semantic ordering, pagination math, readiness, and revision/invalidation belong to backend contracts.
- Canonical read-model owners by bounded context:
  - `wb-sync`: backend export/archive/status/readiness projections; raw WB payloads may power explicit compatibility readers only
  - `wb-clusters`: Postgres-owned advertising sheet/workspace/table/query projections plus backend-owned sync/readiness state
  - `catalog`: durable product catalog projections in Postgres
  - `pricing`: backend/Postgres pricing rules, publish status, and preview read models
  - `stocks`: backend/Postgres warehouse and stock projections
  - `orders`: backend/Postgres order ingestion and operational read models
  - `analytics`: backend/Postgres report/query projections and derived aggregates
...
- `backend/src/wb-clusters/`
  ...
  - `snapshot lifecycle`: product search-text range snapshots, final product advertising snapshot artifacts, snapshot resolver/materializer, and machine-readable readiness metadata; explicit-range interactive reads must return exact/current requested data or explicit unavailability for that same range, while degraded/latest/historical snapshot reads stay explicit compatibility or diagnostic paths instead of silent fallback
...
- `.cursor/rules/backend-postgres-truth.mdc`
