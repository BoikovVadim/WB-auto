...
## Деплой после каждого изменения кода

**После каждого логического завершения задачи запускай продакшен-деплой**, чтобы данные на сайте всегда соответствовали последним изменениям в репозитории.
...
Скрипт сам выполняет полный цикл: `verify:ci` (lint + typecheck + tests + build) → загрузку артефактов на сервер → reload PM2 → `verify:prod` (health-чек на `https://legendgames.space/wb/api/health`).

Альтернатива, если server-first недоступен: `npm run deploy:prod:auto`.

### Когда запускать

- После завершения логически целостной задачи (фича, багфикс, рефакторинг — то, что обычно завершается коммитом).
- Если задача затронула и backend, и frontend — один общий `deploy:server-first:auto` покрывает оба.

### Когда НЕ запускать

- Если правил **только тесты** (файлы `*.test.ts`, `*.spec.ts`, тестовые фикстуры) — продакшн от этого не меняется.
- Если задача явно помечена как незавершённая (есть TODO в коде, который пользователь оставил намеренно, или пользователь сказал «остановись на этом шаге»).
- Если пользователь явно попросил «не деплой» / «только закоммить» / «только покажи диф».

### Что сообщать пользователю

- До запуска: одной строкой «Запускаю `npm run deploy:server-first:auto`».
- После: краткий итог — прошёл ли `verify:prod`, статус PM2-процесса `wb-automation-backend` (`online`/нет).
- При ошибке деплоя: НЕ повторяй слепо. Покажи вывод, разберись в причине (типичные: упал lint/typecheck/тесты, health-чек не прошёл, проблема со ssh-ключом).

### Дополнительная справка

- Полная документация по деплою: [docs/deploy.md](docs/deploy.md).
- Все доступные deploy-скрипты — в `scripts/` и в `"scripts"` корневого `package.json` (`deploy:prod`, `deploy:prod:backend`, `deploy:prod:frontend`, `deploy:server-first:*`).
- Конфиг деплоя: `.env.deploy.local` (если нужно переопределить дефолты из `.env.deploy.example`).
